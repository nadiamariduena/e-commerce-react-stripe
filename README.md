...
> You can see that first we have the 422 error due to the card authentication (maybe because we are still on testing mode), then after that comes the message **cart empty** 🔴, you can also see few warnings(i debugged and now there s only the 422 alert)
...
# Lets MIMIC a succesful order (in case you didnt add a real credit card in commercejs)
...
### [Using pushState for browser and SEO-friendly websites](https://www.newmediacampaigns.com/blog/using-pushstate-for-browser-and-seo-friendly-websites)

>Using the pushState method, you can manipulate the browser's history object on-the-fly. Doing this provides the user the ability to use the browser buttons to navigate through the added states, even though the browser has not reloaded the page.
